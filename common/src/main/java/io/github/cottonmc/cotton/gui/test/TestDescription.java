...
public class TestDescription extends SyncedGuiDescription {
	private static final Identifier TEST_MESSAGE = new Identifier("libgui", "io/github/cottonmc/cotton/gui/test");
	...
	public TestDescription(ScreenHandlerType<?> type, int syncId, PlayerInventory playerInventory, ScreenHandlerContext context) {
		...
		buttonB.setOnClick(() -> slot.setIcon(new TextureIcon(new Identifier("libgui", "saddle.png"))));

		root.add(buttonB, 5, 3, 4, 1);
		TextureIcon testIcon = new TextureIcon(new Texture(new Identifier("libgui", "icon.png")));
		...
		getRootPanel().streamChildren()
				.forEach(child -> {
					if (child instanceof WItemSlot wis) {
						...
						wis.setInputFilter(stack -> !stack.isOf(Items.APPLE));
					}
				});
